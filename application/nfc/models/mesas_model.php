...
class Mesas_Model extends CI_Model {
	...
	function get_m($options = array(),$flag=0)
	{
		...
		if(isset($options['_id']))
			$this->db->where('m._id', $options['_id']);
		if(isset($options['estado']))
			$this->db->where('m.estado', $options['estado']);
		if(isset($options['descripcion']))
			$this->db->like('m.descripcion', $options['descripcion']);
		if(isset($options['created_at']))
			$this->db->where('m.created_at', $options['created_at']);
		if(isset($options['updated_at']))
			$this->db->where('m.updated_at', $options['updated_at']);
		...
		$this->db->select("m.*, tg.descripcion as estado_descripcion");
		$this->db->from("mesas as m");
		$this->db->join("tabgral as tg","tg._id = m.estado");
		$query = $this->db->get();
